<!DOCTYPE html>
<html lang="es">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="css/ejercicio14.css">
    <script src="https://kit.fontawesome.com/eee8b9a576.js" crossorigin="anonymous"></script>
    <link href="https://unpkg.com/aos@2.3.1/dist/aos.css" rel="stylesheet">
    <title>Gran Hotel</title>
</head>
<body class="p-0 m-0 overflow-x-hidden">
    
<?php require_once 'auxiliares/header.php';

    if(isset($_GET['id_habitacion'])){
        $habitacion = infoHabitacion($_GET['id_habitacion']);
    }
?>

<?php if(!esRecepcionista()): ?>
    <meta http-equiv="refresh" content="0;url=habitaciones.php">
<?php endif; ?>

<main class ="pt-36 md:pt-56 lg:p-0">
    <section class="flex justify-center p-6">
        <form id="miFormulario" action="habitaciones.php" method="POST" class="w-full grid lg:grid-cols-3 lg:w-3/6 bg-color-azul-marino color-gris-crema font-bold p-32 rounded-3xl justify-center" enctype="multipart/form-data" novalidate>
         <input type="hidden" name="id_habitacion" value="<?=$habitacion[0]['id_habitacion']; ?>">
    
            <div class="flex col-span-3 flex-col">
                <div class='flex-col w-full lg:flex-row flex text-center  justify-between items-center '>
                    <label for="habitacion" class="pr-3">Nombre de la habitación:</label>
                    <input type="text" name="nombre" id="registro" value="<?php echo htmlspecialchars($habitacion[0]['nombre']) ?>" placeholder="Escriba el número de habitación" class=" mt-6 lg:mt-0 border-2 border-black color-azul-marino">
                </div>
                <div class="w-64 flex justify-between">
                    <p class="text-red-600 hidden" name="error">El número de habitación tiene que ser un número entero</p>
                </div>
            </div>

            <div class="flex col-span-3 flex-col">
                <div class='flex-col w-full lg:flex-row flex text-center  justify-between items-center '>
                    <label for="capacidad" class="pr-3 pt-6">Capacidad:</label>
                    <input type="text" name="capacidad" id="registro" value="<?php echo htmlspecialchars($habitacion[0]['capacidad']); ?>" placeholder="Escriba la capacidad de la habitación" class="mt-6 border-2 border-black color-azul-marino">
                </div>
                <div class="w-64 flex justify-between">
                    <p class="text-red-600 hidden" name="error">Capacidad no válida (tiene que ser mayor que 0 y un número entero)</p>
                </div>
            </div>

            <div class="flex col-span-3 flex-col">
                <div class='flex-col w-full lg:flex-row flex text-center  justify-between items-center '>
                    <label for="precio" class="pr-3 pt-6">Precio por noche:</label>
                    <input type="text" name="precio" id="registro" value="<?php echo htmlspecialchars($habitacion[0]['precio']); ?>" placeholder="Precio de la habitación por noche" class="mt-6 border-2 border-black color-azul-marino">
                </div>
                <div class="w-64 flex justify-between">
                    <p class="text-red-600 hidden" name="error">Precio no válido (tiene que ser un número mayor que 0)</p>
                </div>
            </div>

            <div class="flex col-span-3 flex-col">
                <div class='flex-col w-full lg:flex-row flex text-center  justify-between items-center '>
                    <label for="descripcion" class="pr-3 pt-6">Descripción:</label>
                    <input type="text" name="descripcion" id="registro" value="<?php echo htmlspecialchars($habitacion[0]['descripcion']); ?>" placeholder="Escriba la descripción de la habitación" class="mt-6 border-2 border-black color-azul-marino">
                </div>
                <div class="w-64 flex justify-between">
                    <p class="text-red-600 hidden" name="error">La descripción no puede estar vacía</p>
                </div>
            </div>

            <div class="text-center col-span-3 justify-center items-center w-full pt-16">
                <button type="submit" name="editarHabitacion" value="<?= $habitacion[0]['id_habitacion'] ?>" class="text border-white border-2 p-3 bg-color-bronce-metalico rounded-full w-32 animate-pulse" id="boton">Enviar</button>
            </div>
        </form>
    </section>
</main>

<?php require_once './auxiliares/footer.php'; ?>
<!-- <script src="./js/popupregistro.js" defer></script>
<script src="./js/registro_habitaciones.js" defer></script> -->
</body>
</html>
